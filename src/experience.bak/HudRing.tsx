 ...
 */
function InnerMist({ count }: { count: number }) {
  const ref = useRef<THREE.Points>(null)
...
function GoldenEnergy({ count }: { count: number }) {
  const ref = useRef<THREE.Points>(null)
...
export default function HudRing({ tier }: HudRingProps) {
  const fogCount = tier === 'low' ? 180 : tier === 'medium' ? 280 : 400
  const mistCount = tier === 'low' ? 40 : tier === 'medium' ? 60 : 80
  const goldCount = tier === 'low' ? 80 : tier === 'medium' ? 140 : 200

  return (
    <group>
      <VolumetricFog count={fogCount} />
      <InnerMist count={mistCount} />
      <GoldenEnergy count={goldCount} />
    </group>
  )
}
